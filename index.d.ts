declare module 'seneca-extend' {
  ...
  namespace SNS {
    ...
    interface Instance {
      ...
      addAsync(pattern: string, cmdFun: AddCmdFun): SNS.Instance;
      // addAsync({role:xxx,cmd:xxx}, function)
      addAsync(
        pattern: { role: string; cmd: string },
        cmdFun: AddCmdFun,
      ): SNS.Instance;
      // addAsync(role, cmd, function)
      addAsync(role: string, cmd: string, cmdFun: AddCmdFun): SNS.Instance;
      ...
      throwError(msg: string, code?: number, detail?: any): void;
      useAsync(input: any, options?: any, name?: string): SNS.Instance;
    }
