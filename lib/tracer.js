...
module.exports = function tracerPlugin(opts = {}) {
  ...
  seneca.inward((ctx, data) => {
    ...
    ctx.seneca.actAsync = (...args) => {
      const { role, cmd, params } = parseArgs('act', args);
      const msg = {};
      inject(span.context(), msg);
      const { __tracer__ } = msg;

      return seneca.actAsync.apply(ctx.seneca, [
        { role, cmd, params, __tracer__ },
      ]);
    };
  ...
  seneca.outward((ctx, data) => {
    ...
    if (data.out && data.out.error_code !== 0) {
      ...
      ctx.span.setTag('error_code', data.out.error_code);
      ctx.span.setTag('error_msg', data.out.error_msg);
      ctx.span.log(data);
    }
